...
impl Strategy for QuoteStrategy {
    fn generate_quotes(symbol: Symbol, state: &State) -> Option<(Order, Order)> {
        if let  Some((bid, ask)) = state.bbo_levels[symbol] {
            ...
            Some((
                Order::new(symbol, Side::Buy, OrderKind::Limit, bid_opx, Decimal::ONE, TimeInForce::GoodUntilCancel, None),
                Order::new(symbol, Side::Sell, OrderKind::Limit, ask_opx, Decimal::ONE, TimeInForce::GoodUntilCancel, None),
            ))
        } else {
            None
        }
